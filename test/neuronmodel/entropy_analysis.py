...
def main():
    ...
    for f in os.listdir("entropy_data"):
        print(f'Loading: {os.path.join("entropy_data", f)}')
        df = pd.read_csv(os.path.join("entropy_data", f))
        dfs.append(df)
    ...
    avgs = [df["EntropyAccuracy"].mean() for df in dfs]
    ...
    print(f"Average accuracy: {sum(avgs) / len(avgs)}")
    ...
    avgs = [df["NormalAccuracy"].mean() for df in dfs]
    print(f'Average stdev: {sum(stdevs) / len(stdevs)}')
    print(f"Average maximal accuracy: {sum(maxs)/len(maxs)}")
    print(f"Average accuracy: {sum(avgs) / len(avgs)}")
    print(f"Minimal accuracy: {min(mins)}")
